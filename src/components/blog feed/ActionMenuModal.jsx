...
export default function ActionMenusModal({ blogId }) {
  const { dispatch } = useBlogs(); // Access the dispatch function from the useBlogs hook
  const { api } = useToken(); // Access the API instance from the useToken hook
  ...
  const handleDeleteBlog = async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        dispatch({ type: actions.blogs.POST_DELETED, data: blogId });
